...
### 2026-02-21：电流环（dq）框架接入

- HostCmd：新增 `I` 电流环指令
  - `I<iq_A>`：设置 `Iq_ref`（单位 A，限幅 ±3A）并请求使能电流环（offset 未完成会等待）
  - `I`（不带参数）：关闭电流环并关 PWM 输出
- 控制周期：跟随 ADC Injected ISR（20kHz，`dt=50us`）
- 控制对象：`Id_ref=0`，`Iq_ref` 由 `I` 命令设置；PI 参数默认 `Kp=0.5655`、`Ki=703.7`（积分项内部会乘 `dt`）
- 电压输出：PI 输出单位为 **V**，在 `Vbus=12V` 下做限幅（`|Ud/Uq| <= Vbus*(1/sqrt(3))`），再归一化为 per-unit 送入 SVPWM
- 观测：新增 `D3` 页输出 `Id/Iq/Ud_pu/Uq_pu`
- 注意：当前电角 `theta_e` 使用主循环 1kHz 更新的编码器角（先跑框架验证；后续要做更高频/更低延迟的角度获取）
...
θ,Ud (占空比),万用表实测电压 U对V,万用表实测电压 U对W,采样电流反馈 Ia (A),采样电流反馈 Ib (A)
0,0.05,0.02V,0.02V,-0.09,0.12
...
## 2026-03-10：
电流环给 I0.3A，速度在250rad/s左右达到平衡；给I0.4A，速度在460rad/s左右平衡
## 2026-03-10：更正 F1 对数扫频注入策略（当前实现不冻结速度环）

- 更正 2026-03-03 记录：当前程序中，`F1/F0` 控制的对数扫频仍然叠加在 `Iq_cmd` 侧，但扫频期间已经**不再冻结速度环更新**。
